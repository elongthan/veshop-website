...
export async function downloadCatalogPdf({ products, categoryLabel, settings }) {
  ...
  for (const p of products) {
    ...
    if (p.image_url) {
      try {
        ...
        doc.rect(margin + fit.offsetX, blockTop + fit.offsetY, fit.w, fit.h);
  ...
  const footerLine1 = [settings.site_name, settings.phone1, "veshop.com.sg/contact"].filter(Boolean).join("  ·  ");
