...
export async function downloadCatalogPdf({ products, categoryLabel, settings }) {
  ...
  for (const p of products) {
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    const introLines = p.short_description ? doc.splitTextToSize(p.short_description, textWidth) : [];
    ...
    const textHeight = nameLines.length * 13 + 6
      ...
      + (descLineCount ? descLineCount * 11 + 6 : 0);
    const blockHeight = Math.max(imgBox, textHeight) + 20;
